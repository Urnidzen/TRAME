# TRAME - Système de Combat et de Création Simple

**Codex Officiel** - Système de jeu de rôle tactique et narratif.

## 📚 Les Livrets

| Document | Version | Description |
|----------|---------|-------------|
| [Livret 1 - Mécanismes](https://urnidzen.github.io/TRAME/Livret1-Mecanismes.v0.86.html) | v0.86 | Règles de résolution, combat, magie et exploration |
| [Livret 2 - Création](https://urnidzen.github.io/TRAME/Livret2-Creation.v0.74.html) | v0.74 | Création et évolution des personnages |
| [Livret 3 - Équipement](https://urnidzen.github.io/TRAME/Livret3-Equipement.v0.79.html) | v0.79 | Armures, armes, objets et artisanat |
| [Livret 4 - Rencontres](https://urnidzen.github.io/TRAME/Livret4-Rencontres.v0.80.html) | v0.80 | Équilibrage des rencontres et guide du MJ |
| [Livret 5 - Bestiaire](https://urnidzen.github.io/TRAME/Livret5-Bestiaire.v0.80.html) | v0.80 | Créatures et adversaires |
| [Fiche de Personnage](https://urnidzen.github.io/TRAME/Fiche%20de%20personnage.v0.90.html) | v0.90 | Fiche interactive en ligne |

## 🎲 Philosophie du Système

TRAME abandonne les Points de Vie (PV) traditionnels au profit d'une logique d'**Opportunités et Conséquences**. Le combat devient une discussion tactique où les joueurs cherchent à identifier les failles adverses pour placer des coups décisifs.

**Points clés :**
- ⚔️ **Matrice Martiale** : 12 statistiques de combat (3 Types × 3 Cibles)
- 🎯 **Conséquences narratives** : Pas de dégâts aléatoires, effets réalistes (blessures, handicaps)
- ⚡ **Initiative par Priorité** : Distance &gt; Magie &gt; Allonge (pas de jets d'initiative)
- 🛡️ **Érosion tactique** : Le surnombre pénalise la défense

## 📜 Licence et Droits d'auteur

**© 2026 Urnidzen**

Ce système de jeu est mis à disposition sous licence **Creative Commons Attribution - Pas d'Utilisation Commerciale - Partage dans les Mêmes Conditions 4.0 International** ([CC BY-NC-SA 4.0](https://creativecommons.org/licenses/by-nc-sa/4.0/deed.fr)).

### ✅ Vous êtes libres de :
- **Partager** : copier et redistribuer le matériel sur tout support
- **Adapter** : remixer, transformer et créer des scénarios, fiches, outils
- **Utiliser** : jouer en privé, en association, en convention (gratuitement)

### ❌ Conditions et interdictions :
- **Attribution** : Vous devez créditer "Urnidzen" et lier vers cette page
- **Pas d'Utilisation Commerciale** : Vente, Kickstarter, applications payantes interdites sans autorisation écrite préalable
- **Partage Identique** : Si vous modifiez, vous devez partager sous la même licence

**Les illustrations, portraits et visuels restent la propriété exclusive de l'auteur.**

Pour toute demande d'utilisation commerciale ou question de licence : [ajoutez votre email ou moyen de contact ici]

## 🗺️ Sitemap

Ce site est indexé via un [sitemap.xml](https://urnidzen.github.io/TRAME/sitemap.xml) pour les moteurs de recherche.

---
*Dernier update : Février 2026*
